...
pub fn neutral_icon() -> Result<Icon> {
    build_icon(None, false, false)
}

pub fn battery_icon(percent: u8, charging: bool) -> Result<Icon> {
    build_icon(Some(percent), charging, false)
}

pub fn inactive_battery_icon(percent: u8) -> Result<Icon> {
    build_icon(Some(percent), false, true)
}
...
pub fn inactive_text_icon(percent: u8) -> Result<Icon> {
    let mut pixels = vec![0u8; TEXT_SIZE * TEXT_SIZE * 4];
    draw_number(
        &mut pixels,
    ...
    Icon::from_rgba(pixels, TEXT_SIZE as u32, TEXT_SIZE as u32)
        .context("failed building inactive text tray icon")
}
...
fn build_icon(percent: Option<u8>, charging: bool, inactive: bool) -> Result<Icon> {
    let mut pixels = vec![0u8; SIZE * SIZE * 4];

    draw_battery_shell(&mut pixels);

    match percent {
        Some(level) => draw_battery_fill(&mut pixels, level, charging, inactive),
...
fn draw_battery_fill(pixels: &mut [u8], percent: u8, charging: bool, inactive: bool) {
    let color = if inactive {
        [148, 163, 184, 255]
    } else {
        level_color(percent, charging)
    };
...
#[cfg(test)]
mod tests {
    use super::{battery_icon, inactive_battery_icon, inactive_text_icon, neutral_icon, text_icon};
    ...
    fn icon_generation_works() {
        ...
        assert!(inactive_battery_icon(50).is_ok());
    }
    ...
    fn text_icon_generation_works() {
        ...
        assert!(inactive_text_icon(50).is_ok());
    }
}
